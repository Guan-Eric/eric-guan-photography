...
export const tenants = sqliteTable("tenants", {
  ...
  mediaQuotaBytes: integer("media_quota_bytes").notNull().default(10_737_418_240),
...
export const listingPages = sqliteTable("listing_pages", {
  id: text("id").primaryKey(),
  tenantId: text("tenant_id").notNull(),
  orderId: text("order_id")
    .notNull()
...
export const listingDomains = sqliteTable("listing_domains", {
  id: text("id").primaryKey(),
  tenantId: text("tenant_id").notNull(),
  listingPageId: text("listing_page_id")
    .notNull()
...
export const membershipInvites = sqliteTable("membership_invites", {
  id: text("id").primaryKey(),
  tenantId: text("tenant_id")
    .notNull()
    .references(() => tenants.id),
...
/** Magic-link tokens for the agent portal (legacy; still consumed for one release). */
export const agentLoginTokens = sqliteTable("agent_login_tokens", {
  id: text("id").primaryKey(),
  tenantId: text("tenant_id").notNull(),
  email: text("email").notNull(),
  token: text("token").notNull(),
...
/** Email OTP challenges for agent portal sign-in (primary). */
export const agentOtpChallenges = sqliteTable("agent_otp_challenges", {
  id: text("id").primaryKey(),
  tenantId: text("tenant_id").notNull(),
  email: text("email").notNull(),
