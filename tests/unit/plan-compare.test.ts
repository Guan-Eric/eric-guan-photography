...
describe("plan compare math", () => {
  ...
  it("lists feature rows for every purchasable plan", () => {
    ...
    expect(labels).toContain("Share kit");
    expect(labels).toContain("Gallery reports");
    ...
    const share = planCompareRows().find((row) => row.label === "Share kit");
    expect(share?.values.starter).toBe(true);
    expect(share?.values.growth).toBe(true);
    expect(share?.values.studio).toBe(true);
    const upsells = planCompareRows().find((row) => row.label === "In-gallery upsells");
    expect(upsells?.values.starter).toBe(false);
    expect(upsells?.values.studio).toBe(true);
    expect(upsells?.values.payg).toBe(true);
  });
});
